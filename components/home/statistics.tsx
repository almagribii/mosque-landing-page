...
export default function Statistics() {
  const [counts, setCounts] = useState({ jamaah: 0, activities: 0, volunteers: 0, years: 0 })
  ...
  useEffect(() => {
    ...
    const targetCounts = { jamaah: 5200, activities: 48, volunteers: 150, years: 15 }
    const duration = 2000
    const steps = 60
    const increment = {
  ...
  return (
    <section id="statistics" className="py-20 bg-gradient-to-r from-primary/10 via-background to-accent/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-4xl font-bold text-center text-foreground mb-16">Pencapaian Kami</h2>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat, index) => (
            <div
              key={stat.label}
              className={`text-center p-8 bg-card rounded-xl border border-border hover:border-primary/50 transition-all duration-500 transform ${
